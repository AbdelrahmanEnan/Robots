...
const int L1 = 5;
const int L2 = 4;
const int I1 = 9; // need to be edited to another PWM pin
const int I2 = 10;
...
float SlaveCoor [2] = {50.0,0.0}; //Must be edited to contain the correct coordinates of the Slave Robot
...
int closeCalc(int x, int y)//coor[0],coor[1]
{//float SlaveCoor [2] = {50.0,0.0};
   int result = 0;
   double mee = sqrt(pow(x,2.0)*1.0 + pow(y,2.0)*1.0);
   double slv = sqrt(pow((x-SlaveCoor[0]),2.0)*1.0 + pow((y-SlaveCoor[1]),2.0)*1.0);
   if(mee > slv)
   {
       result = 2;
   }
...
void loop(){
    ...
    if (App.available() > 0)
    {
        ...
        if (received == '0')
        {
            ...
            {
                delay(400);
                coor [0] = App.read();//x coordinates
                coor [1] = App.read();//y coordinates
                Serial.println("Got it ");
                Serial.print(coor [0]);
                Serial.println(coor [1]);
            }
            int closest = closeCalc(coor[0],coor[1]);
            if (closest == 1)
            {
            ...
            {
              Serial.print ("ERRRRRRroooooooRRRRR");
            }
        }
        while (App.available() > 0)
        {
